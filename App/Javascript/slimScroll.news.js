  ...
  jQuery.fn.extend({
    newsScroll: function(options) {
      ...
      var defaults = {
        ...
        railColor : '#333',
        railOpacity : '0.2',
        railClass : 'newsScrollRail',
        barClass : 'newsScrollBar',
        wrapperClass : 'newsScrollDiv',
      ...
      var isOverPanel, isOverBar, isDragg, queueHide, barHeight, percentScroll,
        ...
        if (me.parent().hasClass('newsScrollDiv'))
        {
            //check if we should scroll existing instance
            if (scroll)
            {
                //find bar and rail
                bar = me.parent().find('.newsScrollBar');
                rail = me.parent().find('.newsScrollRail');
        ...
        function dsphinx_autoscrollContent()
        {
          var pixeltoscroll;
        


		$(this).everyTime(200,function(i) {   // require jquery.timers
			if (isOverPanel)
				return;
			...
			var t = me[0].scrollHeight - me.outerHeight();
			
			dsphinx++;
            dsphinx += 1;
			if (dsphinx > t ) {
				dsphinx = pixeltoscroll = 1 ;
			}
			
			pixeltoscroll = dsphinx  + 3;
			scrollContent(pixeltoscroll ,true,true);
//			$("#message").text(" new scroll " + pixeltoscroll + "   to "  + dsphinx + " apo " +  t );
		});
        }
        ...
        function scrollContent(y, isWheel, isJump)
        {
          ...
          // scroll content
          me.scrollTop(delta);
	//		$("#message").text(" scrolling  " +" apo " +  delta + " " + ( me[0].scrollHeight - me.outerHeight()) );

          // ensure bar is visible
         	if (isOverPanel)				// dsphinx
		 		showBar();
...
//		dsphinx_autoscrollContent();
  ...
  jQuery.fn.extend({
    newsScroll: jQuery.fn.newsScroll
  });
